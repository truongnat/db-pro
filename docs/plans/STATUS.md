...
| Table Data Editor Hardening | feature/table-data-editor-completion | — | IMPLEMENTING | Stable RowIdentity/ChangeSet mutations, PK edits via original WHERE + final SET, typed filters, multi-sort, scoped grid layout, conflict recovery, and render lookup maps implemented; provider/runtime/UI evidence and metadata expansion pending |
