...
| Agent Workflow | feature/agent-workflow-hardening | — | REVIEW | Typed provider tool calls, bounded multi-step orchestration, executor/service boundary, run/session/document routing, stale-safe refresh, confirmation pauses, ephemeral tool results, idempotency caching, step limits, explicit retry, and native tool-activity/patch-preview panel; PostgreSQL/SQLite/live-provider evidence pending |
