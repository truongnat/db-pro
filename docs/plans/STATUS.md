...
|---|---|---|---|---|
| Core Safety Hardening | feature/core-safety-hardening | — | IMPLEMENTING | Explain/export/mutation safety, lexical SQL statement boundaries, token-aware destructive SQL classification, atomic multi-statement mutation routing, SQLite and PostgreSQL operation deadlines with transaction rollback, SSH host-key/tunnel safety and matching password-auth test path, SSH credentials separated from metadata, provider-correct backup credential handling, SSH-aware backup with atomic no-overwrite publication, safe PostgreSQL user-management SQL, compensating connection lifecycle/secret cleanup, duplicate-connect cleanup retry, persisted backup secret references, and checked pagination; PostgreSQL fixture live evidence PASS, SSH/native UI evidence pending |
