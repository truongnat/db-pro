...
| Agent Workflow | main | — | RUNTIME_VERIFY | Typed provider tool calls, bounded multi-step orchestration, executor/service boundary, run/session/document routing, stale-safe refresh, confirmation pauses, ephemeral tool results, idempotency caching with collision detection, failure caching, safety recheck on approval, step limits, explicit retry, and native tool-activity/patch-preview panel; live provider and native viewport evidence remain |
