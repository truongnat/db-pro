...
**Runtime evidence:** `cytoscape-renderer.test.ts` +1 (tap vs dbltap routing on the real renderer), `cytoscape-view.test.tsx` (8), `er-diagram-navigation.test.tsx` (2); CDP harness +1 check pair at 500 & 1000 (`singleTapFocus` / `doubleTapOpens`). All 171 ER tests green.
