...
- Connection list/connect/query worker paths now call `ConnectionApi`/`QueryApi` instead of reaching through service internals. `TableDataApi`, `ExportApi`, `BackupApi` and `UserApi` now expose typed native-facing operations. Connection create/update/delete/test commands are also available in the shared runtime worker, and native UI now has create/test form validation; edit flow and progress/cancellation contracts remain.
