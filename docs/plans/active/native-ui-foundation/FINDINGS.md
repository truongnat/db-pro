...
- Query results now cross the runtime/UI boundary as typed columns and cells, with NULL/boolean/number/text/JSON/bytes rendering. The current grid is intentionally capped at 500 rendered rows; virtualization, column resize, sorting and filtering remain next.
