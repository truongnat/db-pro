...
2. **Timestamp-without-time-zone never gains invented timezone semantics.**
   `format_naive_timestamp` (`crates/infrastructure/src/postgres/query_mapper.rs:549`) emits
   `%Y-%m-%dT%H:%M:%S%.6f` with no marker; `format_utc_instant` (`:540`) emits the `Z` form for an
...
`CellValue` (`crates/core/src/domain/query.rs:55-95`) is the single domain representation; the decoder
...
| `INTERVAL` | `Interval` | `N mons N days HH:MM:SS.ffffff` (microseconds) | `Text` | writable | `format_interval` (`:560`), `providers/40` |
...
| `INET`/`CIDR` | `Inet` | `address/prefix` (full prefix kept) | `Text` | writable | `decode_inet` (`:489`), `providers/40` |
...
Implementation: `decode_textual_value` (`:417`), `binary_payload_is_text` (`:443`),
`is_text_like_type_name` (`:460`). The class list is pinned by the unit test
...
| Temporal **mutation parameters** | binders accept the canonical strings and fail explicitly on anything they cannot parse (`bind_params_fails_on_invalid_uuid` `crates/infrastructure/src/postgres/query_mapper.rs:740`, `bind_params_fails_on_invalid_datetime` `:747`) — no silent coercion | #52/#62 boundary |

## 3. The serialization contract (A4)

| Boundary | Shape | Proof |
|---|---|---|
| Domain → in-process UI (the **shipping** channel) | `UiQueryResult` (`crates/ui/src/runtime.rs:555`) via `map_query_result` (`crates/native-app/src/translate.rs:850`) and `map_cell` (`:660`); one `UiCell` class per domain class | `query_result_translation_keeps_every_field_and_value_class`, `map_cell_keeps_one_ui_class_per_domain_value_class` |
| Domain → JSON (the legacy Tauri DTO) | `QueryResultDto` (`crates/tauri-app/src/dto.rs:257`) with `CellValueDto` (`:322`) as an adjacently tagged `{type, value}` enum; `Int64`/`Decimal` as JSON strings | `query_result_dto_serializes_the_locked_camel_case_shape`, `…_covers_every_value_class_tag`, `…_never_exposes_precision_sensitive_values_as_numbers`, `…_matches_the_checked_in_contract_fixture` |
