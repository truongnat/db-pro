...
| Transaction control — **execution**, not classification | **measured gap, 2026-09-15, #147:** classification is covered; execution is not, and it is wrong. A batch that carries its own `COMMIT` escapes the app-owned transaction, so the documented all-or-nothing property silently stops holding. Live PostgreSQL fixture, `execute_transaction(["BEGIN", "INSERT …", "COMMIT", "SELECT no_such_column …"])` → `phase=Statement index=3 outcome=RolledBack` **with 1 row surviving**; the control (same failing statement, no explicit control) → `outcome=RolledBack` with **0 rows surviving**. Both envelopes say `RolledBack`, so the first tells the user a rollback happened while an earlier write is committed. Add `BEGIN`/`COMMIT`/`ROLLBACK` to a batch only after the disposition in `docs/release/evidence/v01-runtime/providers/51-multistatement-contract-criterion-audit.md` is settled; #147 stays open on it |
