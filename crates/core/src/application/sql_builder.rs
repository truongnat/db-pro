...
pub fn build_count(schema: &str, table: &str, filters: &[TableFilter]) -> (String, Vec<QueryParam>) {
    let (where_clause, params) = build_where(filters);
...
pub fn build_update(
    ...
    let set_parts: Vec<String> = columns.iter().map(|c| format!("{} = ?", quote_ident(c))).collect();
    let pk_where: Vec<String> = pk_columns.iter().map(|c| format!("{} = ?", quote_ident(c))).collect();
...
fn build_where(filters: &[TableFilter]) -> (String, Vec<QueryParam>) {
    if filters.is_empty() {
...
mod tests {
    ...
    fn select_no_filters_no_sorts() {
        let (sql, params) = build_select("public", "users", &[], &[], 50, 0);
    ...
    fn select_with_filters_and_sorts() {
        ...
        let (sql, params) = build_select("public", "users", &filters, &sorts, 25, 50);
        assert_eq!(
            sql,
    ...
    fn select_with_null_filters() {
        ...
        let (sql, params) = build_select("public", "users", &filters, &[], 50, 0);
        assert_eq!(
            sql,
    ...
    fn count_with_filter() {
        ...
        let (sql, params) = build_count("public", "users", &filters);
    ...
    fn insert_basic() {
        ...
        let (sql, params) = build_insert("public", "users", &columns, &values);
        assert_eq!(
            sql,
    ...
    fn update_with_pk() {
        ...
        let (sql, params) = build_update("public", "users", &columns, &values, &pk_columns, &pk_values);
        assert_eq!(
            sql,
    ...
    fn delete_with_composite_pk() {
        ...
        let (sql, params) = build_delete("public", "order_items", &pk_columns, &pk_values);
        assert_eq!(
            sql,
    ...
    fn multi_sort() {
        ...
        let (sql, _) = build_select("public", "users", &[], &sorts, 50, 0);
