...
// ═══════════════════════════════════════════════════════════════════════════
// Batch execution / transaction rollback tests
// ═══════════════════════════════════════════════════════════════════════════

#[tokio::test]
async fn execute_batch_all_valid_commits() {
    let (connector, handle) = setup_fixture().await;

    let statements = vec![
        "INSERT INTO categories (name, description) VALUES ('Batch1', 'first')".into(),
        "INSERT INTO categories (name, description) VALUES ('Batch2', 'second')".into(),
    ];

    let affected = connector.execute_batch(&handle, &statements).await.unwrap();
    ...
    let result = connector
        ...
        .unwrap();
    let json = serde_json::to_value(&result.rows[0].0[0]).unwrap();
    assert_eq!(json, serde_json::json!({"type": "int64", "value": 2}));
}

#[tokio::test]
async fn execute_batch_middle_failure_rolls_back_all() {
    let (connector, handle) = setup_fixture().await;

    let count_before = connector
        .query(&handle, "SELECT COUNT(*) AS cnt FROM categories", &[])
        .await
        .unwrap();
    let json_before = serde_json::to_value(&count_before.rows[0].0[0]).unwrap();

    let statements = vec![
        "INSERT INTO categories (name, description) VALUES ('ShouldRollback', 'x')".into(),
        "THIS IS NOT VALID SQL".into(),
        "INSERT INTO categories (name, description) VALUES ('AlsoShouldRollback', 'y')".into(),
    ];
    ...
    let count_after = connector
        ...
        .await
        .unwrap();
    let json_after = serde_json::to_value(&count_after.rows[0].0[0]).unwrap();
    ...
    assert_eq!(json_check, serde_json::json!({"type": "int64", "value": 0}));
}

#[tokio::test]
async fn execute_batch_first_failure_rolls_back() {
    let (connector, handle) = setup_fixture().await;

    let statements = vec![
        "NOT VALID AT ALL".into(),
        "INSERT INTO categories (name) VALUES ('AfterFailure')".into(),
    ];
    ...
    assert_eq!(json, serde_json::json!({"type": "int64", "value": 0}));
}

#[tokio::test]
async fn execute_batch_empty_statements() {
    let (connector, handle) = setup_fixture().await;
    let result = connector.execute_batch(&handle, &[]).await.unwrap();
    assert_eq!(result, 0);
}
