...
impl QueryHistoryRepository for SQLiteMetaStore {
    async fn save(&self, connection_id: &ConnectionId, sql: &str, result: &QueryResult) -> Result<(), DbError> {
        ...
        self.actor
            .raw_query(
                "INSERT INTO query_history (id, connection_id, sql, executed_at, duration_ms, row_count) VALUES (?1, ?2, ?3, ?4, ?5, ?6)".into(),
                vec![
                    id,
    ...
    async fn list(&self, connection_id: &ConnectionId, limit: u32) -> Result<Vec<QueryHistory>, DbError> {
        let rows = self
            .actor
            .raw_query(
                "SELECT id, connection_id, sql, executed_at, duration_ms, row_count FROM query_history WHERE connection_id = ?1 ORDER BY executed_at DESC LIMIT ?2".into(),
