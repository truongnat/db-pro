...
impl DbProApp {
    ...
    fn draw_sidebar_contents(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            let selector_w = (ui.available_width() - HEADER_ACTIONS_W - ui.spacing().item_spacing.x).max(72.0);
            ...
            let chevron_galley = ui
                .painter()
                .layout_no_wrap(chevron, font_icon(ICON_XS), self.theme.text_secondary);
            let name_max = (sel_rect.width() - SPACE_XS * 2.0 - chevron_galley.size().x - SPACE_XS).max(24.0);
                    ...
                    match self.activity {
                        ...
                        Activity::Tasks => {
                            if self.pending_destructive_task_id.is_some() {
                                ui.checkbox(&mut self.saved_task_confirm_destructive, "Confirm destructive task run");
