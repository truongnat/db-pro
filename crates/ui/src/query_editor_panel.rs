use super::query_editor_support::{
    apply_completion_item, draw_completion_explanation, draw_rich_hover_popup, draw_signature_help,
};
use super::query_view::{format_query_document, prediction_replacement_range};
use super::*;
use crate::editor::{
    CompletionIntent, CompletionItemKind, CompletionTriggerKind, EditorInteractionPolicy, SqlDialect, SqlEditor,
};
    ...
    pub(super) fn draw_query_editor(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some((trigger_kind, is_manual_trigger)) = completion_request {
            ...
            let context = CompletionContext {
                ...
                schema_summary: &self.schema.explorer.schema,
        ...
        let signature = if response.focused && !doc.completion.is_open {
            self.schema.explorer.schema_symbol_index.signature_help(
                doc.buffer.text(),
                doc.cursor.offset,
                &active_schema,
                dialect,
        ...
        if let Some(confirmed) = doc.hover_state.confirmed_token() {
            ...
            if let Some(rich) = self.schema.explorer.schema_symbol_index.rich_hover(
                sql,
                confirmed.range,
                &active_schema,
                dialect,
