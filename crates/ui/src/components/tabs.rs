...
pub struct TabItemStyle {
    pub font_id: egui::FontId,
    pub text_color: egui::Color32,
}

impl TabItemStyle {
    pub fn segmented(is_active: bool, is_hovered: bool, theme: &DbProTheme) -> Self {
    ...
    pub fn underline(is_active: bool, is_hovered: bool, theme: &DbProTheme) -> Self {
        let font_id = if is_active {
...
pub struct TabTrackerAnimation;

impl TabTrackerAnimation {
    pub fn animate_indicator(ctx: &egui::Context, track_id: egui::Id, target: Rect) -> (f32, f32) {
        let x = ctx.animate_value_with_time(track_id.with("x"), target.left(), TAB_TRANSITION_SECS);
    ...
    pub fn animate_pill(ctx: &egui::Context, track_id: egui::Id, target: Rect) -> Rect {
        let (x, w) = Self::animate_indicator(ctx, track_id, target);
        Rect::from_min_size(egui::pos2(x, target.top()), Vec2::new(w, target.height()))
    }

    pub fn animate_underline(ctx: &egui::Context, track_id: egui::Id, target: Rect) -> Rect {
        let (x, w) = Self::animate_indicator(ctx, track_id, target);
        Rect::from_min_size(
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                for (idx, tab_name) in self.tabs.iter().enumerate() {
                    ...
                    let (rect, clicked) = self.paint_item(ui, tab_name, is_active);
                    tab_rects.push(rect);
                    if clicked {
            ...
            if let Some(target) = tab_rects.get(*self.selected).copied() {
                let pill = TabTrackerAnimation::animate_pill(ui.ctx(), track_id, target);
                self.paint_active_pill(ui, pill, pill_shape_idx);
            }
        });
    }

    fn paint_item(&self, ui: &mut Ui, tab_name: &str, is_active: bool) -> (Rect, bool) {
        let style = TabItemStyle::segmented(is_active, false, &self.theme);
        let galley = ui
            .painter()
            .layout_no_wrap(tab_name.to_owned(), style.font_id, style.text_color);

        let item_width = (galley.size().x + 24.0).max(48.0);
        let (rect, resp) = ui.allocate_exact_size(Vec2::new(item_width, 28.0), Sense::click());
        let resp = resp.on_hover_cursor(CursorIcon::PointingHand);

        let active_style = TabItemStyle::segmented(is_active, resp.hovered(), &self.theme);
        let text_pos = Pos2::new(
            rect.center().x - galley.size().x * 0.5,
            rect.center().y - galley.size().y * 0.5,
        );
        ui.painter().galley(text_pos, galley, active_style.text_color);
...
impl<'a> UnderlineTabs<'a> {
    ...
    pub fn show(self, ui: &mut Ui) {
        ...
        if let Some(target) = tab_rects.get(*self.selected) {
            self.paint_active_underline(ui, track_id, *target);
        }
    }

    fn paint_item(&self, ui: &mut Ui, tab_name: &str, is_active: bool) -> (Rect, bool) {
        let style = TabItemStyle::underline(is_active, false, &self.theme);
        let galley = ui
            .painter()
            .layout_no_wrap(tab_name.to_string(), style.font_id.clone(), style.text_color);

        let item_width = galley.size().x + 24.0;
        let (rect, resp) = ui.allocate_exact_size(Vec2::new(item_width, 32.0), Sense::click());
        let resp = resp.on_hover_cursor(CursorIcon::PointingHand);
        ...
        let active_style = TabItemStyle::underline(is_active, resp.hovered(), &self.theme);
        ui.painter().text(
            rect.center(),
            Align2::CENTER_CENTER,
            tab_name,
            active_style.font_id,
            active_style.text_color,
        );

        (rect, resp.clicked())
    }

    fn paint_active_underline(&self, ui: &Ui, track_id: egui::Id, target: Rect) {
        let underline = TabTrackerAnimation::animate_underline(ui.ctx(), track_id, target);
        ui.painter()
