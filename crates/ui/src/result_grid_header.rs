    ...
    pub(super) fn draw_grid_header(
        ...
        ui.horizontal(|ui| {
            ...
            for (visual_idx, &col_idx) in order.iter().enumerate() {
                ...
                let sort_active = table_sort_active || self.table_data.grid_sort_column == Some(col_idx);
                let sort_desc = if let Some(sort) = table_sort {
                    sort.descending
                } else {
                    self.table_data.grid_sort_desc
                };
                ...
                context_action_menu(ui, &col_resp, theme, |ui, close_menu| {
                    ...
                    if !self.table_data.grid_hidden_columns.is_empty()
                ...
                if col_resp.clicked() && !divider.dragged() {
                    if self.workspace.active_tab == WorkspaceTab::Table && self.table_view == TableView::Data {
                        ...
                        self.set_table_or_grid_sort(
                            result,
                            col_idx,
                            if self.table_data.grid_sort_desc {
                                None
                            } else {
                                Some(true)
                            },
                ...
                if divider.drag_started() {
                    self.table_data.grid_column_widths = widths.to_vec();
                    self.table_data.grid_columns_user_resized = true;
                }
                if divider.dragged() {
                    self.table_data.grid_column_widths[col_idx] =
                        (self.table_data.grid_column_widths[col_idx] + divider.drag_delta().x).clamp(60.0, 1000.0);
                }
        ...
        if reset_order_req {
            self.table_data.grid_column_order = (0..result.columns.len()).collect();
        }
        if reset_widths_req {
