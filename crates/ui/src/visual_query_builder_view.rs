...
impl DbProApp {
    ...
    pub(super) fn draw_visual_query_builder(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if ghost_button(ui, "Clear", self.theme).clicked() {
                self.query_editor.visual_builder.clear();
            }
        });
        if let Some(error) = &self.query_editor.visual_builder.error {
        ...
        ui.horizontal(|ui| {
            egui::ComboBox::from_id_salt("vqb_add_table")
                .selected_text(if self.query_editor.visual_builder.add_table.is_empty() {
                    "Select table…"
                } else {
                    &self.query_editor.visual_builder.add_table
                })
                .show_ui(ui, |ui| {
                    for table in &self.schema_explorer.schema.table_details {
                        ...
                        ui.selectable_value(
                            &mut self.query_editor.visual_builder.add_table,
                            key,
                            format!("{}.{}", table.schema, table.name),
                        );
                    }
            ...
            if secondary_button(ui, "Add", self.theme).clicked() {
                self.query_editor.visual_builder.add_selected_table(dialect);
            }
        });
            ...
            .enumerate()
        {
            ui.horizontal(|ui| {
                ...
                if ghost_button(ui, "Remove", self.theme).clicked() {
                    self.query_editor.visual_builder.remove_table(idx, dialect);
                }
            });
        }
        ...
        ui.horizontal(|ui| {
            ui.add(
                egui::TextEdit::singleline(&mut self.query_editor.visual_builder.join_table).hint_text("schema.table"),
            );
            ui.add(
                egui::TextEdit::singleline(&mut self.query_editor.visual_builder.join_left).hint_text("left_alias.col"),
            );
            ui.add(
                egui::TextEdit::singleline(&mut self.query_editor.visual_builder.join_right)
                    .hint_text("right_alias.col"),
            );
            if secondary_button(ui, "Add INNER JOIN", self.theme).clicked() {
                self.query_editor.visual_builder.add_join(dialect);
            }
            ...
            .enumerate()
        {
            ui.horizontal(|ui| {
                ...
                if ghost_button(ui, "×", self.theme).clicked() {
                    self.query_editor.visual_builder.remove_join(idx, dialect);
                }
            });
        }
        ...
        ui.checkbox(&mut self.query_editor.visual_builder.model.select_star, "SELECT *");
        ui.horizontal(|ui| {
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.col_ref).hint_text("alias.column"));
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.col_alias).hint_text("AS alias"));
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.col_agg).hint_text("AGG optional"));
            if secondary_button(ui, "Add column", self.theme).clicked() {
                self.query_editor.visual_builder.add_column(dialect);
            }
        });
        for (idx, col) in self
            .query_editor
            ...
            .enumerate()
        {
            ui.horizontal(|ui| {
                ...
                if ghost_button(ui, "×", self.theme).clicked() {
                    self.query_editor.visual_builder.remove_column(idx, dialect);
                }
            });
        }
        ...
        ui.horizontal(|ui| {
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.where_left).hint_text("alias.col"));
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.where_op).hint_text("="));
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.where_value).hint_text("value"));
            if secondary_button(ui, "Add WHERE", self.theme).clicked() {
                self.query_editor.visual_builder.add_where(dialect);
            }
        });
            ...
            .enumerate()
        {
            ui.horizontal(|ui| {
                ...
                if ghost_button(ui, "×", self.theme).clicked() {
                    self.query_editor.visual_builder.remove_predicate(idx, dialect);
                }
            });
        }
        ui.horizontal(|ui| {
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.order).hint_text("alias.col"));
            ui.checkbox(&mut self.query_editor.visual_builder.order_desc, "DESC");
            if secondary_button(ui, "Add ORDER", self.theme).clicked() {
                self.query_editor.visual_builder.add_order(dialect);
            }
            ui.label("LIMIT");
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.limit).desired_width(60.0));
            ui.label("OFFSET");
            ui.add(egui::TextEdit::singleline(&mut self.query_editor.visual_builder.offset).desired_width(60.0));
            if ghost_button(ui, "Apply limit", self.theme).clicked() {
                self.query_editor.visual_builder.apply_limit_offset();
                self.query_editor.visual_builder.refresh_preview(dialect);
            }
        });
        ...
        egui::ScrollArea::vertical().max_height(160.0).show(ui, |ui| {
            ui.label(RichText::new(&self.query_editor.visual_builder.sql_preview).monospace());
        });
    }
    ...
    fn apply_visual_builder_sql(&mut self) {
        let dialect = self.visual_builder_dialect();
        self.query_editor.visual_builder.refresh_preview(dialect);
        if self.query_editor.visual_builder.sql_preview.is_empty() {
            return;
        }
        self.set_active_query_text(self.query_editor.visual_builder.sql_preview.clone());
    ...
    fn import_visual_builder_from_editor(&mut self) {
        let sql = self
            ...
            .unwrap_or_default();
        let dialect = self.visual_builder_dialect();
        match self.query_editor.visual_builder.import_sql(&sql, dialect) {
            Ok(()) => {
                self.feedback.runtime_message = "Imported supported SELECT into visual builder".into();
            }
            Err(err) => {
                self.query_editor.visual_builder.error = Some(format!("Import refused (keeping text editor): {err}"));
            }
        }
    }
}
