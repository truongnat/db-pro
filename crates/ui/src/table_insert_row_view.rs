...
impl DbProApp {
    ...
    pub(super) fn draw_insert_row_dialog(&mut self, ctx: &egui::Context) {
        ...
        let mut open = self.table.editing.insert_row_open;
        ...
        Dialog::new(&mut open, &title, self.theme)
            ...
            .show_ctx(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if Button::new(self.theme)
                            ...
                            .clicked()
                        {
                            for val in &mut self.table.editing.insert_row_values {
                                val.clear();
                            }
                        }
                            ...
                            .clicked()
                        {
                            for (index, column) in info.columns.iter().enumerate() {
                                if !column.nullable
                                    ...
                                    && self.table.editing.insert_row_values[index].trim().is_empty()
                                {
                                    self.table.editing.insert_row_values[index] =
                        ...
                        {
                            for (index, column) in info.columns.iter().enumerate() {
                                if self.table.editing.insert_row_values[index].trim().is_empty() {
                                    self.table.editing.insert_row_values[index] =
                                        table_editor_values::generate_sample_value(&column.name, &column.data_type);
                                }
                            }
                            ...
                            .show(ui, |ui| {
                                ui.horizontal(|ui| {
                                    ...
                                    if ColumnWritePolicy::read(column).is_writable() {
                                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                            if !self.table.editing.insert_row_values[index].is_empty() {
                                                ...
                                                if clear_btn.on_hover_text("Clear this field").clicked() {
                                                    self.table.editing.insert_row_values[index].clear();
                                                }
                                            }

                                            if column.nullable && self.table.editing.insert_row_values[index] != "NULL"
                                            {
                                                ...
                                                if null_btn.on_hover_text("Set value to literal NULL").clicked() {
                                                    self.table.editing.insert_row_values[index] = "NULL".to_owned();
                                                }
                                            }
                                ...
                                let is_null_val = self.table.editing.insert_row_values[index]
                                    .trim()
                                ...
                                if let Some(block) = write_block {
                                    ...
                                    let val_ref = &mut self.table.editing.insert_row_values[index];
                ...
                if !self.table.editing.insert_row_error.is_empty() {
                    ui.add_space(8.0);
                    Alert::new("Cannot Stage Insert", &self.table.editing.insert_row_error, self.theme)
